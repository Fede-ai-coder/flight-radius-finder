...
import { resolveDestination } from "@/lib/destinationSearch";
...
export default function HomePage() {
  ...
  const departureSuggestions = useMemo(
    () => Array.from(new Set(DEPARTURE_OPTIONS.flatMap((option) => [option.label, ...option.aliases]))),
    [],
  );
  ...
  const flights = useMemo(() => {
    ...
    return buildMockFlights(nearbyAirports, destinationLabel || destinationCodes[0] || destination.trim(), date);
  }, [nearbyAirports, destination, date, destinationCodes, destinationLabel]);
        ...
        <div className="space-y-4 rounded-2xl bg-white p-4 shadow">
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. LAX"
            />
            <p className="mt-2 text-xs text-slate-500">{destinationResolution.description}</p>
          </div>
