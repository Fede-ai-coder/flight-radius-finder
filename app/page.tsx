...
type SortKey = "from" | "to" | "date" | "airline" | "departure" | "arrival" | "duration" | "stops" | "price";
...
type AirportWithDistance = (typeof AIRPORTS)[number] & { distanceKm: number };
...
function SortableFlightTable({ title, flights, destinationCodes, sortKey, sortDirection, onSort }: { title: string; flights: FlightResult[]; destinationCodes: string[]; sortKey: SortKey; sortDirection: SortDirection; onSort: (key: SortKey) => void }) {
  ...
  const header = (label: string, key: SortKey) => (
    <button type="button" onClick={() => onSort(key)} className="font-semibold hover:text-blue-700">
  ...
  return (
    <div className="mt-5">
      <div className="mb-2 flex items-center justify-between gap-2">
        ...
        <p className="text-sm text-slate-500">{sortedFlights.length} result{sortedFlights.length === 1 ? "" : "s"}</p>
      </div>
      <div className="overflow-x-auto">
        <table className="min-w-full text-left text-sm">
          <thead>
            <tr className="border-b text-slate-600">
              <th className="p-2">{header("From", "from")}</th>
              <th className="p-2">{header("To", "to")}</th>
              <th className="p-2">{header("Date", "date")}</th>
              <th className="p-2">{header("Airline", "airline")}</th>
              <th className="p-2">{header("Departure", "departure")}</th>
              <th className="p-2">{header("Arrival", "arrival")}</th>
          ...
          <tbody>
            {sortedFlights.map((flight) => {
              const isAlternativeArrival = destinationCodes.length > 0 && !destinationCodes.includes(flight.to);
              return (
                <tr key={flight.id} className="border-b last:border-b-0">
                  <td className="p-2">{flight.fromCode} ({flight.fromCity})</td>
                  <td className="p-2">{flight.to}{isAlternativeArrival && <span className="ml-2 rounded-full bg-amber-100 px-2 py-0.5 text-xs text-amber-800">alternative</span>}</td>
...
export default function HomePage() {
  ...
  const exactDepartureAirports = useMemo<AirportWithDistance[]>(() => exactDepartureCodes.map((code) => AIRPORTS.find((airport) => airport.code === code)).filter((airport): airport is NonNullable<typeof airport> => Boolean(airport)).map((airport) => ({ ...airport, distanceKm: 0 })), [exactDepartureCodes]);
  ...
  const resultOriginCount = originSummaries.filter((summary) => summary.status === "found" || summary.status === "partial").length;
  ...
  const highlightedAirports = useMemo(() => originSummaries.filter((summary) => summary.status === "found" || summary.status === "partial").map((summary) => { const airport = originAirportByCode.get(summary.origin); return airport ? { ...airport, resultCount: summary.resultCount, cheapestPrice: summary.cheapestPrice, currency: summary.currency } : null; }).filter((airport): airport is NonNullable<typeof airport> => Boolean(airport)), [originSummaries, originAirportByCode]);
  ...
  const secondaryPolygons = useMemo(() => {
    const polygons: { points: Coordinate[]; color: string; fillOpacity: number }[] = [];
  ...
  async function handleSearchFlights() {
    ...
    try {
      const response = await fetch("/api/flights/search", {
        ...
        body: JSON.stringify({ origins: nearbyOriginCodes, destinations: destinationCodes, dateFrom: date, dateTo: normalizedDateTo, returnDateFrom: returnDate, returnDateTo: normalizedReturnDateTo, tripType, adults, maxResults, nonStop }),
      });
      if (!response.ok) throw new Error("Flight search failed");
      const data = (await response.json()) as FlightSearchResponse;
      setFlights(data.outboundFlights ?? data.flights ?? []);
      setReturnFlights(data.returnFlights ?? []);
      setOriginSummaries(data.originSummaries ?? []);
      setReturnOriginSummaries(data.returnOriginSummaries ?? []);
      setSearchMeta(data.searchMeta ?? null);
      setReturnSearchMeta(data.returnSearchMeta ?? null);
      ...
      setReturnFlights([]);
      setOriginSummaries([]);
      setReturnOriginSummaries([]);
      setSearchMeta(null);
      setReturnSearchMeta(null);
    ...
    <main className="mx-auto min-h-screen max-w-6xl p-4 md:p-8">
      ...
      <section className="mt-6 grid gap-6 lg:grid-cols-3">
        ...
        <div className="space-y-4 rounded-2xl bg-white p-4 shadow">
          <div><label className="mb-2 block text-sm font-medium">Trip type</label><div className="grid grid-cols-2 gap-2"><button type="button" onClick={() => setTripType("oneWay")} className={`rounded-lg border px-3 py-2 text-sm ${tripType === "oneWay" ? "border-blue-600 bg-blue-600 text-white" : "border-slate-300"}`}>One-way</button><button type="button" onClick={() => setTripType("roundTrip")} className={`rounded-lg border px-3 py-2 text-sm ${tripType === "roundTrip" ? "border-blue-600 bg-blue-600 text-white" : "border-slate-300"}`}>Round trip</button></div></div>
          ...
          {!isReturnDateRangeValid && <p className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">Return to must be the same as or after Return from.</p>}
          <p className="text-xs text-slate-500">Outbound: {dateRangeLabel}{tripType === "roundTrip" ? ` · Return: ${returnDateRangeLabel}` : ""}. Maximum 14 days per leg.</p>
      ...
      <section className="mt-6 rounded-2xl bg-white p-4 shadow">
        ...
        {searchMeta?.wasLimited && <div className="mb-4 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900"><span className="font-semibold">Search limited:</span> checked {searchMeta.searchedCombinations} of {searchMeta.requestedCombinations} outbound combinations. {searchMeta.wasDateRangeLimited ? `Outbound date range was limited to ${searchMeta.maxDateRangeDays} days. ` : ""}{returnSearchMeta?.wasLimited ? `Return checked ${returnSearchMeta.searchedCombinations} of ${returnSearchMeta.requestedCombinations}. ` : ""}Try drawing smaller areas, choosing more specific airports, or narrowing date ranges.</div>}
        {alternateArrivalCount > 0 && <div className="mb-4 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900">{alternateArrivalCount} visible outbound result{alternateArrivalCount === 1 ? "" : "s"} arrive at an airport different from the requested destination area {destinationLabel}.</div>}
        ...
        <SortableFlightTable title="Outbound flights" flights={visibleOutboundFlights} destinationCodes={destinationCodes} sortKey={sortKey} sortDirection={sortDirection} onSort={handleSort} />
        {tripType === "roundTrip" && <SortableFlightTable title="Return flights" flights={visibleReturnFlights} destinationCodes={nearbyOriginCodes} sortKey={sortKey} sortDirection={sortDirection} onSort={handleSort} />}
      </section>
    </main>
  );
}
